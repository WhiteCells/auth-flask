...
def user_grid():
    ...
    if "deptId" in request.args:
        ...
        dept_cte = dept_cte.union_all(
            db.session.query(Organization.ID).join(
                dept_cte, Organization.SYORGANIZATION_ID == dept_cte.c.ID
            )
        )
        pagination = (
            User.query.join(Organization, User.organizations)
            .join(dept_cte, Organization.ID == dept_cte.c.ID)
            .filter(*filters)
            .params(dept_id=request.args["deptId"])
            .order_by(*order_by)
...
def syuser_update():
    ...
    if "deptId" in request.json:
        user.organizations = Organization.query.filter(
            Organization.ID == request.json["deptId"]
        ).all()
...
def syuser_update_avatar():
    ...
    unique_filename = f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
    ...
    user.PHOTO = f"/{filepath}"
    db.session.commit()

    return jsonify({"code": 200, "msg": "修改成功"})


@base.route("/uploads/avatars/<filename>", methods=["GET"])
@login_required
def get_user_avatar(filename):
    BASE_DIR = os.path.abspath(os.path.dirname(__file__))  # 获取当前文件的绝对路径
    UPLOAD_FOLDER = os.path.join(BASE_DIR, "uploads/avatars")  # 头像目录的绝对路径
...
def syuser_info():
    ...
    resources += [
        res for role in current_user.roles for res in role.resources if role.resources
    ]
    ...
    return jsonify(
        {
            ...
            "postGroup": current_user.organizations[0].NAME
            if len(current_user.organizations) > 0
            else "",
    ...
    return jsonify(
        {
            "code": 200,
            "msg": "操作成功",
            "roles": [role.to_json() for role in allRoles],
            "user": user.to_json(),
        }
    )
